...
import { DonationHistoryService } from '../donation-history/donationHistory.service';
...
export class DonorsService {
  ...
  async updateDonor(userId: number, body: UpdateDonorsDto) {
    const { weight, unitBlood, address, bloodType, name, avatar, status, dob, gender, provinceName, wardName, street, lastDonation } = body;
    let data: any;
    if (address) {
      const coordinates = await this.geocodingService.getCoordinates(address);
      data = await this.donorsRepository.updateDonor(userId, { weight, unitBlood, address, bloodType, name, avatar, status, dob, gender, provinceName, wardName, street, lastDonation }, { lat: coordinates.lat, lng: coordinates.lon });
    } else {
      data = await this.donorsRepository.updateDonor(userId, { weight, unitBlood, address, bloodType, name, avatar, status, dob, gender, provinceName, wardName, street, lastDonation });
    }
  ...
  async respondDonor(dto: RespondDonorDto) {
    ...
    await this.notificationRepository.updateAcceptStatus(notificationId, action === 'accept');
    ...
    if (notification && notification.hospital) {
      ...
      const payload = {
        ...
        message: action === 'accept' ? 'Người hiến máu đã chấp nhận yêu cầu của bạn.' : 'Người hiến máu đã từ chối yêu cầu.'
      };
