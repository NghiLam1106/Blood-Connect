...
export default function AdminDonorDetail() {
    ...
    return (
      <div className="max-w-3xl mx-auto text-center py-24">
        ...
        <button
          ...
          className="mt-6 px-5 py-2 bg-red-600 text-white text-sm font-bold rounded-xl hover:bg-red-700 transition-colors"
        >
          Quay lại danh sách
        </button>
      </div>
  ...
  const filteredHistories = historyFilter === 'ALL'
    ? histories
    : histories.filter((h) => h.status === historyFilter)

  const acceptedCount = histories.filter((h) => h.status === 'ACCEPTED').length
  const pendingCount = histories.filter((h) => h.status === 'PENDING').length
  const rejectedCount = histories.filter((h) => h.status === 'REJECTED').length
  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      <button
        onClick={() => navigate('/admin/donors')}
        className="flex items-center gap-2 text-sm text-gray-400 hover:text-red-600 font-semibold mb-6 transition-colors"
      >
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 mb-4">
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-gray-400 mt-0.5">ID người dùng: #{donor.userId ?? userId}</p>
      ...
      <div className="grid grid-cols-3 gap-3 mb-4">
        <div className="bg-white rounded-2xl p-4 border border-gray-100 shadow-sm border-l-4 border-l-red-500 text-center">
          <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Lần hiến thành công</p>
          <p className="text-2xl font-extrabold text-red-600">{donor.totalDonations ?? 0}</p>
        </div>
        <div className="bg-white rounded-2xl p-4 border border-gray-100 shadow-sm border-l-4 border-l-blue-500 text-center">
          <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Tỉ lệ phản hồi</p>
          <p className="text-2xl font-extrabold text-blue-600">{formatRate(donor.responseRate)}</p>
        </div>
        <div className="bg-white rounded-2xl p-4 border border-gray-100 shadow-sm border-l-4 border-l-emerald-500 text-center">
          ...
          <p className="text-sm font-extrabold text-emerald-600">{formatDate(donor.lastDonation)}</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 mb-4">
        ...
        <InfoRow icon={<PersonOutlinedIcon style={{ fontSize: 18 }} />} label="Ngày đăng ký" value={formatDate(donor.createdAt)} />
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        ...
        <div className="px-6 pt-5 pb-4 border-b border-gray-100 flex items-center justify-between flex-wrap gap-3">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-bold text-dark">Lịch sử hiến máu</h2>
            <span className="text-[11px] font-bold px-2 py-0.5 rounded-full bg-gray-100 text-gray-500">
              {histories.length} lần
            </span>
          </div>
          <div className="flex gap-2 flex-wrap">
            {FILTER_TABS.map((tab) => {
              const count = tab.key === 'ACCEPTED' ? acceptedCount : tab.key === 'PENDING' ? pendingCount : tab.key === 'REJECTED' ? rejectedCount : histories.length
              return (
                <button
                  key={tab.key}
          ...
          {filteredHistories.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 text-center">
              ...
              <p className="text-xs text-gray-400">
                {historyFilter === 'ALL'
                  ? 'Donor này chưa có lịch sử hiến máu nào.'
