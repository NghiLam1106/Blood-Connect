...
export default function DonorMatching() {
  ...
  useEffect(() => {
    if (!user?.id) return
    setLoading(true)
    getHospitalDonationHistory(Number(user.id))
  ...
  const handleAction = async (id: number, status: 'ACCEPTED' | 'REJECTED') => {
    ...
    try {
      ...
      showToast(
        status === 'ACCEPTED' ? '✅ Đã duyệt lịch sử hiến máu!' : '❌ Đã từ chối lịch sử hiến máu.',
        ...
        <AnimatePresence>
          {toast && (
            <motion.div
              ...
              exit={{ opacity: 0, y: -10 }}
              className={`absolute top-4 right-4 z-50 px-4 py-2.5 rounded-xl text-white text-xs font-bold shadow-lg flex items-center gap-2 ${toast.type === 'success' ? 'bg-green-500' : 'bg-red-500'}`}
            >
              {toast.text}
            </motion.div>
          )}
          ...
          {!loading && !error && filtered.length === 0 && (
            <div className="flex flex-col items-center justify-center py-20 text-center">
              ...
              <p className="text-xs text-gray-500 max-w-xs mx-auto">
                {filter === 'ALL'
                  ? 'Chưa có người hiến máu nào tại bệnh viện.'
            ...
            <div className="flex flex-col gap-3">
              <AnimatePresence>
                {filtered.map((history, index) => {
                  ...
                  const isProcessing = loadingId === history.id
                      ...
                      {isPending && (
                        <div className="flex items-center gap-2 shrink-0">
                          <button
                            ...
                            className="flex items-center gap-1 text-xs font-bold bg-green-500 text-white px-3 py-1.5 rounded-xl hover:bg-green-600 disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-sm"
                          >
                            {isProcessing ? (
                          ...
                          <button
                            onClick={() => handleAction(history.id, 'REJECTED')}
                            disabled={isProcessing || loadingId !== null}
                            className="flex items-center gap-1 text-xs font-bold bg-red-500 text-white px-3 py-1.5 rounded-xl hover:bg-red-600 disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-sm"
                          >
                            {isProcessing ? (
                              <CircularProgress size={11} sx={{ color: 'white' }} />
                            ) : (
